...
function normalizeModel(raw, fallbackId = '') {
    ...
    return {
        id,
        name: firstString(raw?.model_name, raw?.name, raw?.display_name, raw?.id, fallbackId, id),
        family: 'litellm',
        provider: raw?.owned_by || 'litellm',
        aspectRatios,
...
async function fetchOptionalModelInfo(url, key) {
    try {
        const response = await fetch(`${url.replace(/\/+$/, '')}/model/info`, {
            headers: {
...
export async function fetchLiteLLMModels() {
    ...
    if (!response.ok) {
        ...
        throw new Error(`Failed to fetch LiteLLM models: ${response.status} ${text.slice(0, 200)}`);
    }
    ...
    const infoOnly = infoModels
        ...
        .filter(Boolean);
    return dedupeModels([...normalized, ...infoOnly]);
}
