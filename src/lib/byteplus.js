...
export const BYTEPLUS_KEY_STORAGE = 'byteplus_api_key';
export const BYTEPLUS_URL_STORAGE = 'byteplus_base_url';
...
export function setBytePlusApiKey(key) {
  if (typeof window === 'undefined') return;
  localStorage.setItem(BYTEPLUS_KEY_STORAGE, (key || '').trim());
}

export function clearBytePlusApiKey() {
  if (typeof window === 'undefined') return;
  localStorage.removeItem(BYTEPLUS_KEY_STORAGE);
}
...
export function getBytePlusKeyMasked() {
  return maskKey(getBytePlusApiKey());
}
...
async function createVideoTask(modelId, content, options = {}) {
  ...
  if (!apiKey) throw new Error('BytePlus API Key missing. Add it in Settings or the Auth modal.');
  ...
  if (!response.ok) {
    ...
    throw new Error(`BytePlus API Error (${response.status}): ${errText.slice(0, 300)}`);
  }
...
async function pollVideoTask(taskId) {
  ...
  throw new Error(`BytePlus generation timed out after ${Math.round(POLL_TIMEOUT / 1000)}s (${attempt} poll attempts).`);
}

export async function generateVideo(modelId, prompt, options = {}, imageUrl = null) {
  const content = [];
